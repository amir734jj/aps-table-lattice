...
(
  ...
  val _t_ValueType : C_TYPE[T_ValueType] with C_COMBINABLE[T_ValueType] with C_COMPLETE_PARTIAL_ORDER[T_ValueType]
)
  ...
  with C_TABLE_LATTICE[T_TABLE[T_KeyType,T_ValueType],T_KeyType,T_ValueType] {
    ...
    override val v_join = v_combine // intersection
    override val v_meet = v_combine // union
